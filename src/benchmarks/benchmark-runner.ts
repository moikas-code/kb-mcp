...
export class BenchmarkRunner {
  ...
  async runAllBenchmarks(): Promise<Result<BenchmarkSuite>> {
    try {
      ...
      const suite: BenchmarkSuite = {
        ...
        description:
          "Comprehensive performance testing of code analysis capabilities",
  ...
  private async runCoreAnalysisBenchmarks(): Promise<BenchmarkResult[]> {
    ...
    const singleFileResult = await this.runBenchmark(
      ...
      async () => {
        ...
        return await this.analysisEngine!.analyzeFile(
          testFile.path,
          testFile.content,
        );
      },
  ...
  private async runPatternDetectionBenchmarks(): Promise<BenchmarkResult[]> {
    ...
    const antiPatternResult = await this.runBenchmark(
      ...
      async () => {
        ...
        const analysis = await this.analysisEngine!.analyzeProject(
          testProject.path,
        );
        return analysis.success
          ? analysis.data.patterns.filter((p) => p.type === "anti_pattern")
          : [];
      },
    ...
    const designPatternResult = await this.runBenchmark(
      ...
      async () => {
        ...
        return analysis.success
          ? analysis.data.patterns.filter((p) => p.type === "design_pattern")
          : [];
      },
    ...
    const codeSmellResult = await this.runBenchmark(
      ...
      async () => {
        ...
        return analysis.success
          ? analysis.data.patterns.filter((p) => p.type === "code_smell")
          : [];
      },
   ...
   * Natural Language Query Benchmarks
   */
  private async runNaturalLanguageQueryBenchmarks(): Promise<
    BenchmarkResult[]
  > {
    const results: BenchmarkResult[] = [];
  ...
  private generateSummary(
    ...
    if (
      results.some((r) => r.memory && r.memory.peakUsage > 1024 * 1024 * 1024)
    ) {
      recommendations.push(
        "Memory usage is high - consider implementing streaming analysis",
      );
    }

    if (results.some((r) => r.metrics.p95Time > r.metrics.averageTime * 3)) {
      recommendations.push(
        "High variance in performance - investigate inconsistent operations",
      );
    }

    if (recommendations.length === 0) {
      recommendations.push(
        "Performance is good - maintain current optimization level",
      );
    }
  ...
  private generateCsvSummary(results: BenchmarkResult[]): string {
    ...
    const rows = results.map((result) => [
      ...
      result.memory
        ? (result.memory.peakUsage / 1024 / 1024).toFixed(2)
        : "N/A",
    ]);
  ...
  private outputResults(suite: BenchmarkSuite): void {
    ...
    console.log(
      `Average Performance: ${suite.summary.averagePerformance.toFixed(2)} ops/s`,
    );
    console.log(
      `Performance Rating: ${suite.summary.performanceRating.toUpperCase()}`,
    );
    ...
    for (const result of suite.results) {
      ...
      const throughput = `${result.metrics.throughput.toFixed(1)} ops/s`.padEnd(
        12,
      );
      const p95Time = `${result.metrics.p95Time.toFixed(1)}ms`.padEnd(12);
      const memoryPeak = result.memory
        ? `${(result.memory.peakUsage / 1024 / 1024).toFixed(1)}MB`
        : "N/A";

      console.log(
        name + category + avgTime + throughput + p95Time + memoryPeak,
      );
    }
  ...
  private async getCpuInfo(): Promise<string> {
    try {
      ...
      const cpuList = cpus();
      return `${cpuList[0].model} (${cpuList.length} cores)`;
    } catch {
      return "Unknown CPU";
    }
  }
