...
async function stopDatabase(_options: any): Promise<void> {
  ...
  try {
    ...
    spinner.fail(`Failed to stop database: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
async function resetDatabase(options: any): Promise<void> {
  ...
  try {
    ...
    spinner.fail(`Failed to reset database: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
async function getDBStatus(projectName: string): Promise<DBStatus> {
  try {
    ...
    // Get port mapping
    const projectDir = path.join(os.homedir(), '.kb-mcp', 'projects', projectName);
    let ports = { falkordb: 6380, redis: 7379 };
    
    try {
      const portData = await fs.readFile(path.join(projectDir, 'ports.json'), 'utf-8');
...
function isContainerRunning(containerName: string): boolean {
  try {
    const result = execSync(`docker ps --format "{{.Names}}" | grep "^${containerName}$"`, {
      encoding: 'utf-8',
      stdio: 'pipe'
    });
  ...
  while (retries < maxRetries) {
    ...
    if (status.containers.falkordb && status.containers.redis) {
      ...
      try {
        ...
        // Test connections
        execSync(`redis-cli -h localhost -p ${redis} -a ${password} ping`, { stdio: 'ignore' });
        execSync(`redis-cli -h localhost -p ${falkordb} -a ${password} ping`, { stdio: 'ignore' });
...
async function saveDBState(projectName: string, state: string): Promise<void> {
  const stateFile = path.join(os.homedir(), '.kb-mcp', 'projects', projectName, 'state.json');
  const stateData = {
    state,
    project: process.cwd(),
 ...
 * Get all KB projects
 */
async function getAllKBProjects(): Promise<Array<{id: string, name: string}>> {
