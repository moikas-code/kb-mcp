...
if (process.versions && (process.versions as any).bun) {
  console.error(
    "ERROR: kb-mcp does not support Bun. Please use Node.js and npm.",
  );
  process.exit(1);
}
...
import { dbCommand, updateCommand } from "./commands/index.js";
...
class UnifiedKBCLI {
  ...
  private setupCommands(): void {
    ...
    this.program
      ...
      .option(
        "-t, --type <type>",
        "Backend type (filesystem or graph)",
        "filesystem",
      )
...
`,
      ...
      spinner.succeed(
        `Knowledge base initialized successfully with ${options.type} backend`,
      );
      console.log(
        chalk.blue(`\nKB created with backend:`),
        chalk.cyan(options.type),
      );
      console.log(
        chalk.gray("Configuration saved to:"),
        chalk.cyan(".kbconfig.yaml"),
      );
      console.log(
        chalk.gray("Structure:"),
        "docs/, notes/, references/, guides/, archive/, active/",
      );
    } catch (error) {
  ...
  private async readCommand(filePath: string): Promise<void> {
    ...
    try {
      ...
      spinner.fail(
        `Failed to read ${filePath}: ${error?.message || String(error)}`,
      );
    }
  }
  ...
  private async writeCommand(
    ...
    if (!finalContent) {
      if (useEditor) {
        ...
        console.log(
          chalk.yellow(
            'Enter content below. Finish with a single "." on a line:',
          ),
        );
        const lines: string[] = [];
    ...
    try {
      ...
      spinner.fail(
        `Failed to write ${filePath}: ${error?.message || String(error)}`,
      );
    }
  }
  ...
  private async searchCommand(query: string, options: any): Promise<void> {
    ...
    try {
      ...
      if (!searchResult.success) {
        throw new Error(
          searchResult.error?.message || String(searchResult.error),
        );
      }
  ...
  private async deleteCommand(filePath: string): Promise<void> {
    ...
    try {
      ...
      console.log(
        chalk.gray(
          `Backend: ${this.backendManager.getBackend()!.getBackendType()}`,
        ),
      );
    } catch (error) {
  ...
  private async statusCommand(): Promise<void> {
    try {
      ...
      console.log(
        "Overall Completion:",
        chalk.cyan(`${status.overall_completion}%`),
      );
      console.log(
        "Critical Issues:",
        chalk.red(status.critical_issues.toString()),
      );

      if (status.last_updated) {
        console.log(
          "Last updated:",
          chalk.gray(new Date(status.last_updated).toLocaleString()),
        );
      }
            ...
            phase.status === "completed"
              ...
              : phase.status === "in_progress"
                ...
                : phase.status === "blocked"
                  ...
                  : chalk.gray;
          console.log(
            `  ${phase.name}: ${statusColor(phase.status)} (${phase.completion}%)`,
          );
          if (phase.notes) {
  ...
  private async switchBackend(type: string): Promise<void> {
    ...
    try {
      const result = await this.backendManager.switchBackend(
        type as "filesystem" | "graph",
      );
      if (!result.success) {
        throw new Error(result.error?.message || String(result.error));
      }
  ...
  private async backendInfo(): Promise<void> {
    try {
      ...
      console.log(
        "Status:",
        healthResult.success ? chalk.green("Healthy") : chalk.red("Unhealthy"),
      );
      ...
      if (config) {
        ...
        if (config.filesystem) {
          ...
          console.log(
            "  enable_versioning:",
            chalk.gray(config.filesystem.enable_versioning),
          );
        }
        if (config.graph) {
          console.log(
            "  connection:",
            chalk.gray(JSON.stringify(config.graph.connection)),
          );
          console.log(
            "  vector_dimensions:",
            chalk.gray(config.graph.vector_dimensions),
          );
        }
      }
  ...
  private async serveCommand(options: any): Promise<void> {
    ...
    try {
      ...
      if (!options.local) {
        console.log(
          chalk.gray(`WebSocket: ws://localhost:${options.wsPort || 8080}/mcp`),
        );
        console.log(
          chalk.gray(
            `SSE: http://localhost:${options.ssePort || 8081}/mcp/events`,
          ),
        );
      }
