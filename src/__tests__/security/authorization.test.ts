...
describe('Authorization Security', () => {
  ...
  describe('Permission Checking', () => {
    test('should allow exact permission match', () => {
      const result = authService.checkPermission(testContexts.editor, 'kb.read');
      expect(result).toBe(true);
    });

    test('should deny missing permission', () => {
      const result = authService.checkPermission(testContexts.viewer, 'kb.write');
      expect(result).toBe(false);
    });

    test('should handle wildcard permissions', () => {
      // Admin has kb.*
      expect(authService.checkPermission(testContexts.admin, 'kb.read')).toBe(true);
      expect(authService.checkPermission(testContexts.admin, 'kb.write')).toBe(true);
      expect(authService.checkPermission(testContexts.admin, 'kb.delete')).toBe(true);
      expect(authService.checkPermission(testContexts.admin, 'kb.admin')).toBe(true);
    });
    ...
    test('should require all permissions when multiple provided', () => {
      const result = authService.checkPermissions(
        testContexts.editor,
        ['kb.read', 'kb.write']
      );
      expect(result).toBe(true);
      
      const result2 = authService.checkPermissions(
        testContexts.editor,
        ['kb.read', 'kb.admin']
      );
    ...
    test('should check any permission with OR logic', () => {
      const result = authService.checkAnyPermission(
        testContexts.viewer,
        ['kb.write', 'kb.read', 'kb.admin']
      );
      expect(result).toBe(true); // Has kb.read
      
      const result2 = authService.checkAnyPermission(
        testContexts.viewer,
        ['kb.write', 'kb.delete', 'kb.admin']
      );
  ...
  describe('Role-Based Access Control', () => {
    ...
    test('should handle role hierarchy', () => {
      ...
      expect(authService.checkPermission(context, 'anything.at.all')).toBe(true);
    });
  ...
  describe('Resource-Based Access Control', () => {
    test('should check resource ownership', () => {
      ...
      expect(authService.checkResourceAccess(
        testContexts.editor,
        resource,
        'write'
      )).toBe(true);
  ...
  describe('Permission Delegation', () => {
    test('should allow permission delegation', () => {
      ...
      const editorWithDelegation = {
        ...testContexts.editor,
        delegated_permissions: authService.getDelegatedPermissions('editor-user'),
      };
      
      expect(authService.checkPermission(
        editorWithDelegation,
        'system.config.read'
      )).toBe(true);
    });
  ...
  describe('Attribute-Based Access Control', () => {
    test('should evaluate attribute-based rules', () => {
      ...
      const result = authService.evaluateABACRule(context, resource, 'read', rule);
      expect(result).toBe(true);
    });
    ...
    test('should handle complex attribute expressions', () => {
      ...
      const result = authService.evaluateABACRule(context, resource, 'approve', rule);
  ...
  describe('Security Context Validation', () => {
    test('should validate required MFA for sensitive operations', () => {
      ...
      for (const op of sensitiveOps) {
        // With MFA
        expect(authService.validateContextForOperation(
          testContexts.admin,
          op
        )).toBe(true);
