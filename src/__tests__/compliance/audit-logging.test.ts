...
import { AuditEvent, SecurityContext, ComplianceConfig } from '@types/index';
    ...
    beforeEach(async () => {
      ...
      for (const event of events) {
        await auditLogger.log({
          ...event,
          resource: 'test',
        } as Partial<AuditEvent>, testContext);
      }
    });
  ...
  describe('Retention Policy', () => {
    ...
    test('should respect retention periods by event type', async () => {
      ...
      const eventTypes: AuditEvent['event_type'][] = [
        'auth', 'authz', 'data_access', 'config_change', 'security', 'error'
      ];
