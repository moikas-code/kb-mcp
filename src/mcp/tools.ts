...
export function createTools(_backendManager: BackendManager): Tool[] {
  return [
    ...
    // Code Analysis Tools
    {
      ...
      description: 'Analyze a codebase to extract entities, relationships, and generate insights',
      inputSchema: {
        type: 'object',
        properties: {
          path: {
          ...
          languages: {
            type: 'array',
            items: { type: 'string' },
            description: 'Programming languages to analyze (e.g., ["typescript", "javascript"])',
...

    {
      ...
      description: 'Find all functions that call a specific function and trace call relationships',
      inputSchema: {
        type: 'object',
        properties: {
          functionName: {
...

    {
      ...
      description: 'Get the inheritance hierarchy for a class, including parents and children',
      inputSchema: {
        type: 'object',
        properties: {
          className: {
          ...
          filePath: {
            type: 'string',
            description: 'Optional: specific file path where the class is defined'
          },
          includeInterfaces: {
            type: 'boolean',
...

    {
      ...
      description: 'Analyze the impact of changing a code entity (function, class, etc.)',
      inputSchema: {
        type: 'object',
        properties: {
          entityName: {
...

    {
      ...
      description: 'Find code patterns similar to a given code snippet using semantic search',
      inputSchema: {
        type: 'object',
        properties: {
          codeSnippet: {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          patternTypes: {
            ...
            items: { 
              type: 'string',
              enum: ['singleton', 'factory', 'observer', 'strategy', 'decorator', 'adapter', 'all']
            },
      ...
      inputSchema: {
        ...
        properties: {
          ...
          refactoringTypes: {
            ...
            items: {
              type: 'string',
              enum: ['complexity', 'duplication', 'naming', 'structure', 'performance']
            },
      ...
      inputSchema: {
        ...
        properties: {
          ...
          debtTypes: {
            ...
            items: {
              type: 'string',
              enum: ['complexity', 'duplication', 'outdated', 'testing', 'documentation', 'security']
            },
...

    {
      ...
      description: 'Generate a high-level architectural overview of the codebase',
      inputSchema: {
        type: 'object',
        properties: {
          path: {
          ...
          focusAreas: {
            ...
            items: {
              type: 'string',
              enum: ['modules', 'dependencies', 'patterns', 'layers', 'components']
            },
    ...
    {
      name: 'find_usage',
      description: 'Find all usages of a function, class, variable, or type across the codebase',
      inputSchema: {
        type: 'object',
        properties: {
          symbolName: {
          ...
          symbolType: {
            type: 'string',
            enum: ['function', 'class', 'interface', 'type', 'variable', 'constant'],
...

    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          includeExported: {
            type: 'boolean',
            description: 'Include exported symbols that appear unused internally',
            default: false
          },
          excludeTests: {
            type: 'boolean',
...

    {
      ...
      description: 'Perform basic security vulnerability analysis on the codebase',
      inputSchema: {
        type: 'object',
        properties: {
          path: {
          ...
          vulnerabilityTypes: {
            ...
            items: {
              type: 'string',
              enum: ['injection', 'xss', 'hardcoded_secrets', 'insecure_functions', 'path_traversal']
            },
...
export async function executeTool(
  ...
  if (toolName.startsWith('analyze_') || toolName.startsWith('find_') || toolName.startsWith('get_') || 
      toolName.includes('pattern') || toolName.includes('refactor') || toolName.includes('debt') ||
      toolName.includes('metrics') || toolName.includes('query') || toolName.includes('usage') ||
      toolName.includes('dependency') || toolName.includes('dead_code') || toolName.includes('security') ||
      toolName.includes('performance') || toolName.includes('architectural')) {
    
    // Check if backend supports graph operations (needed for analysis)
    if (backend.getBackendType() === 'graph') {
      try {
        const { AnalysisEngine } = await import('../analysis/analysis-engine.js');
  ...
  switch (toolName) {
    ...
    case 'analyze_codebase': {
      if (!analysisEngine) {
        throw new Error('Code analysis requires graph backend. Use kb_backend_switch to switch to graph backend.');
      }
        ...
        return {
          ...
          analysis: {
            entities: {
              ...
              functions: result.data.analysis.entities.filter((e: any) => e.type === 'Function').length,
              classes: result.data.analysis.entities.filter((e: any) => e.type === 'Class').length,
              modules: result.data.analysis.entities.filter((e: any) => e.type === 'Module').length
            },
            relationships: result.data.analysis.relationships.length,
            patterns: {
              total: result.data.patterns.length,
              design_patterns: result.data.patterns.filter((p: any) => p.type === 'design_pattern').length,
              anti_patterns: result.data.patterns.filter((p: any) => p.type === 'anti_pattern').length,
              code_smells: result.data.patterns.filter((p: any) => p.type === 'code_smell').length
            },
            technical_debt: {
              total_items: result.data.technicalDebt.summary.totalItems,
              estimated_hours: result.data.technicalDebt.summary.totalEstimatedHours,
    ...
    case 'query_code_graph': {
      if (!analysisEngine) {
        throw new Error('Natural language queries require graph backend. Use kb_backend_switch to switch to graph backend.');
      }
      ...
      try {
        ...
        throw new Error(`Natural language query failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }

    case 'find_patterns': {
      if (!analysisEngine) {
        throw new Error('Pattern detection requires graph backend. Use kb_backend_switch to switch to graph backend.');
      }
      ...
      try {
        ...
        const patterns = analysisResult.data.patterns.filter((pattern: any) => {
          if (args.patternTypes && !args.patternTypes.includes('all')) {
            ...
            return args.patternTypes.some((type: string) => pattern.name.includes(typeMap[type] || type));
          }
          return true;
        }).filter((pattern: any) => 
    ...
    case 'track_technical_debt': {
      if (!analysisEngine) {
        throw new Error('Technical debt analysis requires graph backend. Use kb_backend_switch to switch to graph backend.');
      }
      ...
      try {
        ...
        const filteredDebt = debt.topPriorities.filter((item: any) => {
          ...
          const minLevel = severityOrder[args.severityThreshold as keyof typeof severityOrder] || 2;
          return severityOrder[item.severity as keyof typeof severityOrder] >= minLevel;
        });

        return {
          success: true,
          path: args.path,
          debt_types: args.debtTypes || ['complexity', 'duplication', 'testing'],
    ...
    case 'get_code_metrics': {
      if (!analysisEngine) {
        throw new Error('Code metrics require graph backend. Use kb_backend_switch to switch to graph backend.');
      }
      ...
      try {
        ...
        return {
          ...
          basic_metrics: {
            ...
            average_complexity: analysis.metrics.complexity / analysis.metrics.functions
          },
          complexity: args.includeComplexity ? {
            average: analysis.metrics.complexity / analysis.metrics.functions,
            max: Math.max(...analysis.entities
              .filter((e: any) => e.type === 'Function')
              .map((e: any) => e.metadata.complexity || 0)),
            distribution: this.calculateComplexityDistribution(analysis.entities)
          } : undefined,
          duplication: args.includeDuplication ? {
            duplicate_patterns: patterns.filter((p: any) => p.name.includes('Duplicate')).length,
            estimated_duplicated_lines: patterns
              .filter((p: any) => p.name.includes('Duplicate'))
              .reduce((sum: number, p: any) => sum + (p.metadata.totalLines || 0), 0)
          } : undefined,
        ...
        throw new Error(`Code metrics analysis failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }

    case 'architectural_overview': {
      if (!analysisEngine) {
        throw new Error('Architectural analysis requires graph backend. Use kb_backend_switch to switch to graph backend.');
      }
      ...
      try {
        ...
        const modules = analysis.entities.filter((e: any) => e.type === 'Module');
        const dependencies = analysis.relationships.filter((r: any) => r.type === 'IMPORTS');
        ...
        return {
          ...
          overview: {
            ...
            average_dependencies_per_module: dependencies.length / modules.length,
            architectural_patterns: result.data.patterns
              .filter((p: any) => ['structural', 'architectural'].includes(p.category))
          ...
          structure: {
            modules: modules.slice(0, 20).map((m: any) => ({
              ...
              dependencies: dependencies.filter((d: any) => d.sourceId === m.id).length
            }))
          },
          ...
          metrics: args.includeMetrics ? {
            coupling: this.calculateCoupling(analysis.entities, analysis.relationships),
            cohesion: this.calculateCohesion(analysis.entities, analysis.relationships),
            complexity: analysis.metrics.complexity / analysis.metrics.functions
          } : undefined
        };
      } catch (error) {
 ...
 */
function calculateComplexityDistribution(entities: any[]): Record<string, number> {
...
function calculateCohesion(entities: any[], relationships: any[]): number {
  ...
  for (const cls of classes) {
    const methods = entities.filter(e => 
      e.type === 'Function' && e.metadata.parentClass === cls.name
    );
    const internalCalls = relationships.filter(r => 
      r.type === 'CALLS' && 
    ...
    const cohesion = maxPossibleCalls > 0 ? (internalCalls.length / maxPossibleCalls) * 100 : 100;
