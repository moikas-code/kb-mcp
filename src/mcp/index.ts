...
const wsPort = parseInt(
  args.find((arg) => arg.startsWith("--ws-port="))?.split("=")[1] || "8080",
);
const ssePort = parseInt(
  args.find((arg) => arg.startsWith("--sse-port="))?.split("=")[1] || "8081",
);
const authSecret = args
  .find((arg) => arg.startsWith("--auth-secret="))
  ?.split("=")[1];
...
const serverOptions = {
  ...
  oauth2: oauth2Config.enabled
    ? {
        ...oauth2Config,
        port: parseInt(
          args.find((arg) => arg.startsWith("--oauth2-port="))?.split("=")[1] ||
            "3000",
        ),
      }
    : undefined,
...
async function main() {
  ...
  if (!isLocal) {
    ...
    if (oauth2Config.enabled) {
      ...
      console.error(
        "OAuth2 authentication enabled - obtain token from OAuth2 endpoints",
      );
    } else if (authSecret) {
      console.error(
        "Simple authentication enabled - use Authorization: Bearer <secret>",
      );
    }
  }
