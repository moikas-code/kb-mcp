...
export class SSETransport implements Transport {
  ...
  async start(): Promise<void> {
    return new Promise((resolve, reject) => {
      this.server = this.app.listen(this.options.port || 3000, this.options.host || 'localhost', () => {
        this.logger.info(`SSE MCP server listening on ${this.options.host || 'localhost'}:${this.options.port || 3000}${this.options.path || '/mcp'}`);
        resolve();
      });
